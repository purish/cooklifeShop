...
$().ready( function() {
	...
	var $searchValue = $("#searchValue");// 搜索框
	
	var $listTable = $("#listTable");
	var $selectAll = $("#selectAll");// 复选框（全选）
	var $ids = $("#listTable input[name='ids']");// 复选框（列表项）
	var $contentRow = $("#listTable tr:gt(0)");
	var $sort = $("#listTable a.sort");
	
	var $pageNumber = $("#pageNumber");// 请求的页码
	var $pageSize = $("#pageSize");// 每页显示记录数
	var $searchProperty = $("#searchProperty");// 搜索选项
	var $orderProperty = $("#orderProperty");// 排序属性
	var $orderDirection = $("#orderDirection");// 排序方向（升序/降序）
	...
	// yjw-每页显示记录数
	if($pageSize.val() != ""){
		var $yjwPageSizeOption = $("#pageSizeOption a[val='" + $pageSize.val() + "']");
		$pageSizeOption.removeClass("current");
		$yjwPageSizeOption.addClass("current");
	}
	
	// yjw-搜索项(回显查询条件时设置选中项)
	if($searchProperty.val() != ""){
		var $yjwSearchPropertyOption = $("#searchPropertyOption a[val='" + $searchProperty.val() + "']");
		$searchPropertyOption.removeClass("current");
		$yjwSearchPropertyOption.addClass("current");
	}
